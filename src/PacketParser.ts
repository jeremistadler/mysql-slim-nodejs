...
function readPacketLength(b: Buffer, off: number) {
  const b0 = b[off]
  const b1 = b[off + 1]
  const b2 = b[off + 2]
  if (b1 + b2 === 0) {
...
export class PacketParser {
  ...
  constructor(
    packetHeaderLength: number /* 4 for normal packets, 7 for comprssed protocol packets */
  ) {
