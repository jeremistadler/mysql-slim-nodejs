...
if (!srcDir) {
  var args = [];
  args[0] =
    process.argv[0].indexOf(' ') !== -1
      ? '"' + process.argv[0] + '"'
      : process.argv[0];
  args[1] =
    process.argv[1].indexOf(' ') !== -1
      ? '"' + process.argv[1] + '"'
      : process.argv[1];
...
for (var i = 0; i < codes.length; i++) {
  if (i in codes) {
    stream.write(
      'exports.' +
        codes[i] +
        new Array(alignment - codes[i].length + 1).join(' ') +
        ' = ' +
        i +
        ';\n'
    );
  ...
  if (i in codes) {
    stream.write(
      'exports[' +
        i +
        ']' +
        new Array(alignment - String(i).length + 1).join(' ') +
        " = '" +
        codes[i] +
        "';\n"
    );
...
function appendGlobalErrorCodes(srcDir, codes) {
  ...
  var match = null;
  var num = 0;
  var regexp = /#define +(EE_[A-Z0-9_]+)\s+([0-9]+)/gm;
...
function appendDatabseErrorCodes(srcDir, codes) {
  ...
  var match = null;
  var num = 0;
  var regexp = /#define +(HA_[A-Z0-9_]+)\s+([0-9]+)/gm;
...
function appendSqlErrorCodes(srcDir, codes) {
  ...
  for (var i = 1; i < sections.length; i += 2) {
    ...
    var names = sections[i + 1].match(/^([A-Z0-9_]+)/gm).map(fixupCode);
...
function extractMySqlVersion(srcDir) {
  ...
  return (
    dictionary.MYSQL_VERSION_MAJOR +
    '.' +
